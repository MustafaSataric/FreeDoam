...
local clickFirst = NativeUI.CreateItem("Stavite Pancir", "~g~Stavite Pancir")
    menu:AddItem(clickFirst)
    menu.OnItemSelect = function(sender, item, index)
        if item == clickFirst then
        SetPedArmour(PlayerPedId(), 100)
        notify("~g~Pancir Stavljen.")
        end
    end
end

function SecondItem(menu) 
    local click = NativeUI.CreateItem("Healajte se", "~g~Heal yourself")
    menu:AddItem(click)
    menu.OnItemSelect = function(sender, item, index)
        if item == click then
            SetEntityHealth(PlayerPedId(), 200)
            notify("~g~Healovani ste.")
        end
    end
end
...
function FourthItem(menu) 
   local submenu = _menuPool:AddSubMenu(menu, "~b~Vozila") 
   ...
   local seat = NativeUI.CreateSliderItem("Promijenite Sjediste", seats, 1)
